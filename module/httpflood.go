...
func httpFloodEntry(stopChan chan int, remainFlags []string) error {
	var opts HTTPFloodOpt
	...
	return httpFloodStart(stopChan, &opts)
}

func httpFloodStart(stopChan chan int, opts *HTTPFloodOpt) error {
...
func httpRequest(opts *HTTPFloodOpt, fin chan error) {
